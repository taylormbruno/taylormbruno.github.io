...
$("#daySched").on("click", function(){
    ...
    $("#modalBody0").append("<img src='assets/images/passwordGen.png' class='modalImg'><p>This day scheduler allows the user to save important tasks to a specific time slot. The hour is color coded to represent whether the time is in the past, present, or future.</p><a href='https://github.com/taylormbruno/WorkDayScheduler' class='portLink'>Click here to visit the repository.</a>");
    $("#portModal").modal("show");
});
...
$("#codingQuiz").on("click", function(){
    ...
    $("#modalBody0").append("<img src='assets/images/codingQuizSS1.png' class='modalImg'><p>This coding quiz presents the user with the option to take three seperate quizes: one for HTML, one for CSS, and one for JavaScript. This project creates a score based on the amount of time left when the user completes the quiz. There are deductions in time if the user answers incorrectly. This project saves the high scores in local storage but still needs work to fix a few bugs.</p><a href='https://github.com/taylormbruno/codingQuiz' class='portLink'>Click here to visit the repository.</a>");
    $("#portModal").modal("show");
});
